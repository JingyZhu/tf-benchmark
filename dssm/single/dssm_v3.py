...
flags.DEFINE_integer('test_size', 10, "Number of test batches in one pickle pack.")
...
def feed_dict(Train, batch_idx):
    ...
    # print(query_batch.shape[0], query_in.dense_shape[0])
    # if query_batch.shape[0] != query_in.dense_shape[0]:
    #     print("AHAHAHAHA")
    #     print(query_batch.shape[0], query_in.dense_shape[0])
        # with tf.name_scope('input'):
        #     query_batch = tf.sparse_placeholder(tf.float32, shape=np.array(query_in.dense_shape), name='QueryBatch')
        #     doc_batch = tf.sparse_placeholder(tf.float32, shape=np.array(doc_in.dense_shape), name='DocBatch')
